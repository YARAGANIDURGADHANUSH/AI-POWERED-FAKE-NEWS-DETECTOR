...
const styles = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .container textarea {
    ...
    padding: 12px; font-size: 16px; margin-bottom: 15px; resize: none;
    background: rgba(255,255,255,0.9); color: #111;
    font-family: "Segoe UI", sans-serif; outline: none; display: block;
  }
  ...
  .container button {
    ...
    transition: opacity 0.2s, transform 0.1s; font-family: "Segoe UI", sans-serif;
  }
  ...
  .error-msg {
    margin-top: 15px; color: #ff6b6b; text-align: center; font-size: 14px;
    background: rgba(255,100,100,0.1); padding: 10px; border-radius: 8px;
  }
  .result-box {
    margin-top: 20px; padding: 20px; border-radius: 15px;
    background: rgba(255,255,255,0.07); animation: fadeIn 0.4s ease;
  }
  @keyframes fadeIn {
  ...
  .differences {
    ...
    background: rgba(255,160,0,0.08); border-left: 3px solid #ff9800;
  }
  .differences-title { font-size: 12px; font-weight: 700; color: #ff9800; text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 6px; }
  .diff-item { font-size: 13px; color: #ffd580; margin-bottom: 4px; }
  .sources { margin-top: 15px; }
  .sources-title { font-size: 13px; font-weight: 700; color: #aaa; text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 10px; }
  .source-card {
  ...
  .source-card:hover { background: rgba(255,255,255,0.15); transform: translateY(-2px); }
  .source-card a { color: #4fc3f7; text-decoration: none; font-size: 13px; word-break: break-all; display: block; }
...
export default function FactChecker() {
  ...
  const checkFact = async () => {
    ...
    try {
      const response = await fetch(BACKEND_URL, { // ✅ uses BACKEND_URL
        method: "POST",
      ...
      setResult({
        ...
        sources: (data.sources || []).map((s) => ({
          title: s.url,
          url: s.url,
          credibility: s.credibility ? Math.round(s.credibility) + "%" : "N/A",
        })),
      });
    } catch (err) {
    ...
    <>
      ...
      <div className="app">
        <div className="container">
          ...
          <textarea
            value={claim}
            onChange={(e) => setClaim(e.target.value)}  // ✅ fixed
            ...
            <div className="result-box">
              ...
              {result.differences?.length > 0 && (
                <div className="differences">
                  ...
                  {result.differences.map((d, i) => (  // ✅ fixed
